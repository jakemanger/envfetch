...
## Parallel processing of summarisation functions

In very large extraction tasks, it may be beneficial to use parallel processing
for the summarisation step. However, often with smaller extraction tasks, 
sending data to multiple cores can be more time costly than the benefit of 
summarising in parallel, so parallel procesing is disabled by default. Parallel
processing can be used with the `parallel=TRUE` argument of `envfetch()`. Note,
`future::plan()` can be set before runnning `envfetch` by the user to configure 
advanced parallel processing options (e.g. using clusters or linux forking). See 
https://future.futureverse.org/reference/plan.html. If setting `future::plan()`
manually, ensure the `create_parallel_plan` argument is `FALSE` in `envfetch()`.
...
bm <- microbenchmark::microbenchmark(
  extract_n_time_ranges(n = 256, large_p, large_r),
  envfetch(large_p, large_r, use_cache = FALSE, verbose = FALSE),
  times=10
)
bm
autoplot(bm)
...
In our larger dataset, time to extract changes from X seconds to Y seconds. We 
get an even greater speedup of Zx.
