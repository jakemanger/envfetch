...
vignette: >
  %\VignetteIndexEntry{benchmark-against-existing-tools}
...
of the Australian Bureau of Meteorology. The data is freely downloadable in yearly netcdf format via: https://awo.bom.gov.au/products/historical/soilMoisture-rootZone/4.5,-27.509,134.221/nat,-25.609,134.362/r/d/2023-07-16
...
```{r}
calculate_summary_stat <- function(d, extracted) {
...
extract_with_raster <- function(p, calculate_summary=TRUE) {
  ...
  return(calculate_summary_stat(p, extracted))
}
```
...
catch_errors <- function(fun, n_decades=1) {
  p <- create_p(n_decades = n_decades)
  tryCatch({
      return(fun(p))
  }, error=function(e) {
      return(e$message)
